...
def test(args):
    ...
    news_scoring_t = []
    news_scoring_v = []
    device = torch.device("cuda" if args.enable_gpu and torch.cuda.is_available() else "cpu")
    with torch.no_grad():
    ...
    n_auc = len(AUC)
    auc_mean = float(np.mean(AUC)) if n_auc else 0.0
    mrr_mean = float(np.mean(MRR)) if n_auc else 0.0
    ndcg5_mean = float(np.mean(nDCG5)) if n_auc else 0.0
    ndcg10_mean = float(np.mean(nDCG10)) if n_auc else 0.0

    with open(os.path.join(args.log_dir,f'test_result_{hvd_rank}.txt'),'w') as fout :
        fout.write(str(auc_mean)+' '+str(n_auc)+'\n')
        fout.write(str(mrr_mean)+' '+str(n_auc)+'\n')
        fout.write(str(ndcg5_mean)+' '+str(n_auc)+'\n')
        fout.write(str(ndcg10_mean)+' '+str(n_auc)+'\n')
    ...
    if hvd_size <= 1:
        with open(os.path.join(args.log_dir,'final_result.txt'),'w') as fout :
            ...
            fout.write(str(ndcg10_mean)+' '+str(n_auc)+'\n')
        logging.info(
            "AUC={:.4f} MRR={:.4f} nDCG@5={:.4f} nDCG@10={:.4f} (n={})".format(
                auc_mean, mrr_mean, ndcg5_mean, ndcg10_mean, n_auc
            )
        )
