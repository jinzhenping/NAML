...
class BertTextPooler(nn.Module):
    def __init__(self, config):
        ...
        self.query = nn.Linear(config.bi_hidden_size,1)
    ...
    def forward(self, hidden_states):
        ...
        score = self.query(pooled_output)
        score = self.activation(score.squeeze(2)).unsqueeze(2)
        pooled_output = torch.bmm(pooled_output.permute(0,2,1),score).squeeze(2)
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.t_pooler_new = BertTextPooler(config)
        self.v_pooler_new = BertImagePooler(config)
    ...
    def forward(
        ...
        pooled_output_t = self.t_pooler_new(sequence_output_t)
        pooled_output_v = self.v_pooler_new(sequence_output_v)
...
class news_encoder(BertPreTrainedModel):
    def __init__(self, config, dropout_prob=0.1, default_gpu=True):
        super(news_encoder, self).__init__(config)
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(dropout_prob)
        self.apply(self.init_bert_weights)
    def forward(
...
from encoder_init import build_news_encoder

class mmrec(torch.nn.Module):
    def __init__(self,config,args):
        super(mmrec,self).__init__()
        self.news_encoder = build_news_encoder(config, getattr(args, "from_pretrained", ""), default_gpu=True)
    ...
    def forward(self,news_feature, input_ids, log_ids, log_mask, targets,compute_loss=True):
        ...
        dim = int(self.config.bi_hidden_size)
        #[batch_size,candi,dim]
        imp_news_vecs_t = news_vecs_t[input_ids].view(batch_size,-1,dim)
        imp_news_vecs_v = news_vecs_v[input_ids].view(batch_size,-1,dim)

        user_click_news_t = news_vecs_t[log_ids].view(batch_size,-1,dim)
        user_click_news_v = news_vecs_v[log_ids].view(batch_size,-1,dim)

        user_vector = self.user_encoder(imp_news_vecs_t,imp_news_vecs_v,user_click_news_t,user_click_news_v,log_mask)
