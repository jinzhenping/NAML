...
# -*- coding: utf-8 -*-
"""
기대이미지 NAML 하이퍼파라미터 탐색.

그리드/two-phase/resume 는 NAML/naml_tune_actual.py · CLIP/tune_s2.py 와 동일.
...
from train_expected_image import evaluate_metrics, generate_batch_data_train
...
def main() -> None:
    ap = argparse.ArgumentParser(description="기대이미지 NAML 하이퍼파라미터 탐색 (naml_tune_actual 과 동일 그리드)")
    ...
    print(
        f"[tune expected-image] train={len(all_train_id)} val_rows={len(all_test_id)} "
        f"history={naml_common.MAX_HISTORY_CLICKS} clip_dim={clip_dim}\n"
        f"[tune expected-image] train cache {train_cov}  test cache {test_cov}",
        flush=True,
    )
    if train_cov["n_nonzero"] == 0:
        raise ValueError("학습 기대이미지가 전부 0벡터입니다.")
    print(f"[tune expected-image] log={out_log}", flush=True)
    print(f"[tune expected-image] weights(best MRR)={out_weights}", flush=True)
    ...
    trial_kw = dict(
        ...
        news_v=news_v,
        news_sv=news_sv,
        train_cand_img=train_cand_img,
        test_cand_img=test_cand_img,
        clip_dim=clip_dim,
