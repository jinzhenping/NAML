...
# -*- coding: utf-8 -*-
"""
기대본문 → CLIP text embedding (user×news). B1과 같은 encoder.
...
def main() -> None:
    ...
    print(
        f"[expected-body text] split={args.split}\n"
        f"[expected-body text] dir={expected_dir} files={len(bodies)}\n"
        f"[expected-body text] tsv={tsv_path} pairs={len(pairs)} "
        f"hit={n_hit} missing={n_missing}\n"
        f"[expected-body text] out={out_path}",
        flush=True,
    )
    extract_clip_text_embeddings(
        items,
