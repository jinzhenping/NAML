...
# -*- coding: utf-8 -*-
"""
학습셋 뉴스 실제 본문 → Kandinsky 2.2 prior → CLIP image_embeds (뉴스 단위).
...
from clip_embeddings import default_actual_body_prior_cache_path, resolve_project_path
from route_embeddings import (
    collect_news_ids_from_interaction_tsv,
    extract_prior_actual_bodies,
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Train-set actual news body → Kandinsky prior embeds")
    ...
    print(
        f"[actual-body prior] news_tsv={news_tsv}\n"
        f"[actual-body prior] scope={args.scope} n_ids={len(news_ids)} "
