...
# -*- coding: utf-8 -*-
"""
클러스터·coordinator 정책 없이, impression TSV에 있는 **모든 유저**에 대해
preference + 클릭 히스토리 + 후보 제목만으로 기대본문을 생성합니다.

기본 프롬프트: `body_generation_no_policy.yaml`
기본 preference:
  - train: user_preference/preference/<dataset>/train/
  - test:  user_preference/preference/<dataset>/test/

사전 준비 (각 split별 1회):
  python user_preference/infer_user_preferences.py --split train --mind-dataset-subdir Adressa_2000
  python user_preference/infer_user_preferences.py --split test  --mind-dataset-subdir Adressa_2000

프로젝트 루트에서:

  # train + test 모두 (출력: .../no_policy_all/train, .../test)
  python user_preference/generate_expected_body_all_users_no_policy.py \
    --split both \
    --output user_preference/expected_body/MIND_2000/no_policy_all \
    --mind-dataset-subdir MIND_2000

  # train만
  python user_preference/generate_expected_body_all_users_no_policy.py \
    --split train \
    --output user_preference/expected_body/MIND_2000/no_policy_all_train \
    --mind-dataset-subdir MIND_2000

  # test만 (기본: dev test + *test*final*.tsv 병합, cluster 스크립트와 동일)
  python user_preference/generate_expected_body_all_users_no_policy.py \
    --split test \
    --output user_preference/expected_body/MIND_2000/no_policy_all_test \
    --mind-dataset-subdir MIND_2000

  # 대용량: 배치 분할 (train/test 각각 동일 인덱스 규칙)
...
NO_POLICY_YAML = PROJECT_ROOT / "user_preference" / "body_generation_no_policy.yaml"
EMPTY_POLICY: Dict[str, str] = {}
...
def collect_pairs_all(impression_df, news_dict: dict) -> List[Tuple[int, str]]:
    """클러스터 필터 없이 TSV의 모든 (user, candidate) 쌍."""
...
def resolve_impression_tsv(
    ...
    """(tsv_path, log_label, tmp_merged_to_delete_after_load)"""
    ...
    if len(merged_list) > 1:
        fd, tmp_name = tempfile.mkstemp(prefix="merged_all_no_policy_test_", suffix=".tsv", text=True)
        os.close(fd)
        tmp = Path(tmp_name)
...
def run_split(args: argparse.Namespace, split: str, out_root: Path) -> None:
    ...
    print(f"\n=== split={split} (no policy, all users) ===")
    ...
    if body_prompt_extra:
        print(f"[prompt] dataset {ds}: Norwegian (bokmal) body suffix added", flush=True)
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    for sp in splits:
        if args.split == "both":
            out_root = out_base / sp
        else:
            out_root = out_base
        run_split(args, sp, out_root)
    print("\n전체 완료.")


def main() -> None:
    ap = argparse.ArgumentParser(
        description="정책·클러스터 없이 train/test 전체 유저 기대본문 생성 (body_generation_no_policy.yaml)"
    )
    ap.add_argument(
        "--split",
        type=str,
        ...
        default="both",
        help="처리할 impression split (기본 both)",
    )
    ap.add_argument("--output", type=str, required=True, help="결과 루트 (--split both면 train/, test/ 하위)")
    ...
    ap.add_argument(
        ...
        nargs="*",
        default=None,
        help="테스트 TSV 병합에 추가할 경로",
    )
    ap.add_argument(
        "--use-test-no-merge-final",
        action="store_true",
        help="*test*final*.tsv 병합 생략 (dev test만)",
    )
    ...
    ap.add_argument("--body-generation-yaml", type=str, default=str(NO_POLICY_YAML))
    ap.add_argument(
    ...
    if args.split == "both" and args.preference_base:
        print("오류: --split both 일 때 --preference-base 는 사용할 수 없습니다 (train/test 각각 기본 경로).")
        sys.exit(1)
