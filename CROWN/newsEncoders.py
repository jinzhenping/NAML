...
Input:
 - age:                 [batch_size, news_num] 
 - user_topic_lifetime: [batch_size, news_num]
Algorithm:
...
class FreshnessEncoder(nn.Module):
    def __init__(self, config: Config, base_news_encoder: nn.Module):
        ...
        if config.fusion_method == 'add' or 'gated':
            hidden_dim= base_news_encoder.news_embedding_dim
        else:
    ...
    def bucketize(self, x):
        ...
        log_x = torch.log(x)
        scaled = log_x / torch.log(torch.tensor(60 * 60 * 24.0)) 
...
class LIME(nn.Module):
    def __init__(self, config: Config, base_news_encoder: nn.Module):
        ...
        self.base_news_encoder = base_news_encoder  
        self.freshness_encoder = FreshnessEncoder(config, base_news_encoder)
...
class NewsEncoder(nn.Module):
    def __init__(self, config: Config):
        ...
        # category embedding (pretrained Glove)
        # self.category_embedding = nn.Embedding(num_embeddings=config.category_num, embedding_dim=config.category_embedding_dim)
...
class CROWN(NewsEncoder):
    def __init__(self, config: Config):
        ...
        # self.title_intent_attention = ScaledDotProduct_Attention(config.intent_embedding_dim, config.category_embedding_dim, config.attention_dim) 
