...
class Config:
    def parse_argument(self, argv=None):
        ...
        parser.add_argument('--use_remaining_lifetime_weighting', type=bool, default=False, choices=[True, False], help='Use remaining lifetime guided weighting in LIME')
        ...
        parser.add_argument('--head_num', type=int, default=10, choices=[3, 5, 8, 10, 12, 15, 20], help='Head number of multi-head self-attention')
        ...
        parser.add_argument('--word_embedding_dim', type=int, default=300, choices=[50, 100, 200, 300, 768], help='Word embedding dimension')
        ...
        if self.dataset in ['mind']:
            ...
            self.max_history_num = 50
        elif self.dataset in ['adressa1w']:
            # for Adressa-1W
            self.train_root = '../Adressa-1W/train'
            self.dev_root = '../Adressa-1W/dev'
            self.test_root = '../Adressa-1W/test'
            self.max_history_num = 50
        elif self.dataset in ['ebnerd']:
            # for EB-NeRD
            self.train_root = '../EB-NeRD-LIME/train'
            self.dev_root = '../EB-NeRD-LIME/dev'
            self.test_root = '../EB-NeRD-LIME/test'
            self.max_history_num = 50
        elif self.dataset in ['mind2000']:
            # for the small MIND_2000 sample shipped under dataset/MIND-2000/
            self.train_root = 'dataset/MIND-2000/train'
        ...
        elif self.dataset in ['adressa', 'adressa1w']:
    ...
    def preliminary_setup(self):
        if self.dataset in ['adressa', 'adressa1w']:
            required = [
                self.train_root + '/news.tsv', self.train_root + '/behaviors.tsv',
                self.test_root  + '/news.tsv', self.test_root  + '/behaviors.tsv',
            ]
            if not self.no_dev:
                required += [self.dev_root + '/news.tsv', self.dev_root + '/behaviors.tsv']
            dataset_files = required
        else:
            dataset_files = [
        ...
        if not self.no_dev and not os.path.exists('dev/ref/truth-%s.txt' % self.dataset):
    ...
    def __init__(self, argv=None):
        ...
        if self.dataset in ['adressa']:
            ...
            with open("./category-adressa1w.json", "r") as f:
                category_dict = json.load(f)
                self.id2category = {v: k for k, v in category_dict.items()}
            with open("./topic_wise_lifetime-adressa1w.json", "r") as f:
                self.topic_wise_lifetime = json.load(f)
            ...
            with open("./category-ebnerd.json", "r") as f:
                category_dict = json.load(f)
                self.id2category = {v: k for k, v in category_dict.items()}
            with open("./topic_wise_lifetime-ebnerd.json", "r") as f:
                self.topic_wise_lifetime = json.load(f)
        elif self.dataset in ['mind2000']:
            with open("./category-mind2000.json", "r") as f:
                category_dict = json.load(f)
                self.id2category = {v: k for k, v in category_dict.items()}
