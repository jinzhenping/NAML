...
class Trainer:
    def __init__(self, model, config, corpus, run_index):
        ...
        self.lifetime_type = config.lifetime_type                       # for LIME
        self.fixed_lifetime = config.fixed_lifetime                     # for LIME
        self.category_lifetime_map = config.category_lifetime_map       # for LIME
        ...
        self.test_res_dir = config.test_res_dir + '/#' + str(self.run_index)
        if not os.path.exists(self.test_res_dir):
            os.mkdir(self.test_res_dir)
        ...
        self.best_dev_avg = AvgMetric(0, 0, 0)
        self.epoch_not_increase = 0
        self.gradient_clip_norm = config.gradient_clip_norm
        self.no_dev = config.no_dev
        self.model.cuda()
    ...
    def train(self):
        ...
        for e in tqdm(range(1, self.epoch + 1), desc='Total Epochs'):
            ...
            for batch_idx, (user_ID, user_category, user_subCategory, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, \
                 ...
                 news_freshness, news_user_topic_lifetime) in enumerate(train_dataloader):
                ...
                news_freshness = news_freshness.cuda(non_blocking=True)                               # [batch_size, 1 + negative_sample_num]
                news_user_topic_lifetime = news_user_topic_lifetime.cuda(non_blocking=True)           # [batch_size, 1 + negative_sample_num]
                user_freshness = user_freshness.cuda(non_blocking=True)                               # [batch_size, max_history_num]
                user_user_topic_lifetime = user_user_topic_lifetime.cuda(non_blocking=True)           # [batch_size, max_history_num]

                
                if self.lifetime_type == "fixed":
                    remaining_lifetime = self.fixed_lifetime - news_freshness                         # [batch_size, 1 + negative_sample_num]
                elif self.lifetime_type == "topic_wise":
                    topic_wise_lifetime = self.category_lifetime_map[news_category]                   # [batch_size, 1 + negative_sample_num]
                    remaining_lifetime = topic_wise_lifetime - news_freshness                         # [batch_size, 1 + negative_sample_num]
                elif self.lifetime_type == "user_topic":
                ...
                logits = model(user_ID, user_category, user_subCategory, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, \
                               user_freshness, user_user_topic_lifetime, user_history_mask, user_history_graph, user_history_category_mask, user_history_category_indices, \
                               news_category, news_subCategory, news_title_text, news_title_mask, news_title_entity, news_content_text, news_content_mask, news_content_entity, \
                ...
                if (batch_idx + 1) % 100 == 0 or (batch_idx + 1) == total_batches:
                    print(f'  Epoch {e}/{self.epoch}  batch {batch_idx+1}/{total_batches} ({100*(batch_idx+1)/total_batches:.1f}%)  loss={float(loss):.4f}', flush=True)
            print('Epoch %d : train done' % e, flush=True)
            print('loss =', epoch_loss / len(self.train_dataset), flush=True)
            torch.save({model.model_name: model.state_dict()}, self.model_dir + '/' + model.model_name + '-epoch' + str(e) + '-ckpt')

            if self.no_dev:
                auc, mrr, ndcg5, hit1 = compute_scores(model, self._corpus, self.batch_size * 2, 'test', self.test_res_dir + '/' + model.model_name + '-' + str(e) + '.txt', self._dataset)
                ...
                metric = {'auc': auc, 'mrr': mrr, 'ndcg5': ndcg5, 'hit1': hit1}[self.dev_criterion]
                ...
                if metric >= best_metric:
                    ...
                    torch.save({model.model_name: model.state_dict()}, self.model_dir + '/' + model.model_name + '-' + str(e))
                    self.epoch_not_increase = 0
                    print(f'Best epoch : {e}', flush=True)
                    print(f'Best {self.dev_criterion} : {metric:.4f}', flush=True)
                else:
                    self.epoch_not_increase += 1
                    print(f'Best epoch : {self.best_dev_epoch}', flush=True)
                    print(f'Best {self.dev_criterion} : {best_metric:.4f}  (no improve {self.epoch_not_increase}/{self.early_stopping_epoch})', flush=True)
                torch.cuda.empty_cache()
                if self.epoch_not_increase == self.early_stopping_epoch:
                    print(f'Early stopping at epoch {e}', flush=True)
                    break
                continue

            # validation
            auc, mrr, ndcg5, hit1 = compute_scores(model, self._corpus, self.batch_size * 2, 'dev', self.dev_res_dir + '/' + model.model_name + '-' + str(e) + '.txt', self._dataset)
            self.auc_results.append(auc)
            self.mrr_results.append(mrr)
            self.ndcg5_results.append(ndcg5)
            self.hit1_results.append(hit1)
            ...
            # wandb.log({'valid_auc': auc, 'valid_mrr': mrr, 'valid_ndcg5': ndcg5}, step=e)
...
def distributed_train(rank, model: nn.Module, config: Config, corpus: Corpus, run_index: int):
    ...
    for e in tqdm(range(1, epoch + 1)):
        ...
        # dev
        if rank == 0:
            auc, mrr, ndcg5, hit1 = compute_scores(model.module, corpus, batch_size * 3 // 2, 'dev', dev_res_dir + '/' + model_name + '-' + str(e) + '.txt', config.dataset)
