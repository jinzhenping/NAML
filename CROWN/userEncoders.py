...
class UserEncoder(nn.Module):
    ...
    def forward(self, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, category, subCategory, user_category, user_subCategory, \
...
class CROWN(UserEncoder):
    ...
    def forward(self, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, category, subCategory, user_category, user_subCategory, \
        ...
        candidate_subtopic_embedding = self.news_encoder.subCategory_embedding(subCategory)            # [batch_size, news_num, subCategory_embedding_dim]
        candidate_topic_representation = self.news_encoder.category_affine(torch.cat([candidate_topic_embedding, candidate_subtopic_embedding], dim=2))     # [batch_size, news_num, category_embedding_dim]
        ...
        if self.use_candidate_aware_attn:
            user_topic_embedding = self.news_encoder.category_embedding(user_category)                       # [batch_size, max_history_num, category_embedding_dim]
            user_subtopic_embedding = self.news_encoder.subCategory_embedding(user_subCategory)                 # [batch_size, max_history_num, category_embedding_dim]
            user_topic_representation = self.news_encoder.category_affine(torch.cat([user_topic_embedding, user_subtopic_embedding], dim=2))

            history_embedding, attn_weights = self.candidate_aware_attn(history_embedding, user_topic_representation, candidate_topic_representation, mask=user_history_mask)
