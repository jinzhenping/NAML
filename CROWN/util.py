...
def compute_scores(model, corpus, batch_size, mode, result_file, dataset):
    ...
    dataloader = DataLoader(DevTest_Dataset(corpus, mode), batch_size=batch_size, shuffle=False, num_workers=0, pin_memory=True)
    ...
    model.eval()
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=f'Evaluating ({mode})', unit='batch', dynamic_ncols=True):
            (
