    ...
    def preprocess(config: Config):
        ...
        if not all(list(map(os.path.exists, preprocessed_data_files))):
                    ...
                    with open(os.path.join(prefix, 'news.tsv'), 'r', encoding='utf-8') as news_f:
                        for line in news_f:
                            ...
                            if news_ID not in news_ID_dict:
                                ...
                                abstract_entities = abstract_entities.replace('""', '"')
            ...
            # for Adressa / EB-NeRD
                ...
                splits = [config.train_root, config.test_root] if config.no_dev else [config.train_root, config.dev_root, config.test_root]
                for i, prefix in enumerate(splits):
                    with open(os.path.join(prefix, 'news.tsv'), 'r', encoding='utf-8') as news_f:
                        for line in news_f:
            ...
            prefixes = [config.train_root, config.test_root] if config.no_dev else [config.train_root, config.dev_root, config.test_root]
            prefix_mode = ['train', 'test'] if config.no_dev else ['train', 'dev', 'test']
        ...
        for line in news_lines:
            ...
            if config.dataset in ['mind']:
                ...
                for entity in entities:
                    ...
                    if not WikidataId:
                        continue
                    for offset in entity.get('OccurrenceOffsets', []):
            ...
            for behavior_index, line in enumerate(train_behaviors_f):
                ...
                for click_impression in click_impressions:
                    self.train_behaviors.append([
                        ...
                        behavior_index,                 # [5]
                        positive_freshness_list[0],     # [6]   candidate news freshness                     
                        pos_user_topic_lifetime,        # [7]   pos candidate lifetime                       (seen/unseen/default fallback)
                        neg_user_topic_lifetimes,       # [8]   neg candidate lifetimes                      
                        freshness_list,                 # [9]   list of clicked news freshness               
                        user_topic_lifetime_list        # [10]  list of clicked news topic-lifetimes         
                    ])
            ...
            with open(os.path.join(config.dev_root, 'behaviors.tsv'), 'r', encoding='utf-8') as dev_behaviors_f:
                for dev_ID, line in enumerate(dev_behaviors_f):
                    ...
                    for impression in impressions_str.strip().split(' '):
                        ...
                        topic = self.category_index_to_name[self.news_category[news_index]]
